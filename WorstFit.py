...
for process_id, process_size in p_size.items():
    ...
    # Finding the worst fitting block
    for block_id, block_size in b_size.items():
        if process_size <= block_size and (block_size - process_size) > max_fragment:
            max_fragment = block_size - process_size
            worst_fit_block = block_id
    
    # Allocate the block if found
    if worst_fit_block is not None:
        b_size[worst_fit_block] -= process_size
        allocation[process_id] = worst_fit_block
    else:
